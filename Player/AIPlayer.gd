...
func _physics_process(delta):
	...
	if state == State.JUMP_FINISHED:
		...
		if not jump_pressed:
			if $AINodes.is_obstructed(move_direction):
				...
				reason = 'obstructed'
				jump_pressed = true
			elif $AINodes.is_about_to_fall(move_direction) and not can_fall_to_target():
		...
		elif not $AINodes.is_obstructed(move_direction) and can_fall_to_target(0):
		...
		if jump_physics.mid_air_jumps == 0:
	...
	#if reason != null:
		#var mid_air = bool(jump_pressed and state == State.FALLING)
		#print('%s: jump_pressed = %s%s' % [reason, jump_pressed, ' (mid air)' if mid_air else ''])
		#get_tree().paused = true
...
func can_fall_to_target(buffer=width/2):
	...
	if rel_target.y <= 0:
		...
		var fall_platform = _closest_platform_under(global_position, platform_search_distance)
	...
	var x = 0
	if move_direction != 0:
		var v = sign(move_direction) * MAX_SPEED
		var acc_time = 0

		if abs(velocity.x) < MAX_SPEED:
			# some time is spent accelerating
			var a = sign(move_direction) * ACCELERATION
			# time to reach max speed. If there isn't enough time to reach max speed: clamp.
			acc_time = min(t, (v-velocity.x)/a)
			# distance covered while accelerating
			# s = ut + 1/2at^2
			x = velocity.x*acc_time + 0.5 * a * acc_time*acc_time
	...
	var short_by = (rel_target.x - x) * sign(move_direction)
	var can_reach = bool(short_by < buffer)
	var pos = to_global(Vector2(x, rel_target.y))
